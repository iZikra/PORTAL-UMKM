<x-admin-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-gray-800 dark:text-gray-200 leading-tight">
            ...
            {{ __('Kelola Frequently Asked Questions (FAQ)') }}
        </h2>
            ...
            <div class="overflow-x-auto border border-gray-200 dark:border-gray-700 rounded-lg">
                <table class="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
                    <thead class="bg-gray-50 dark:bg-gray-700/50">
                        {{-- [MODIFIED] Tombol "Tambah FAQ" sekarang berada di dalam header tabel --}}
                        <tr>
                            ...
                            <th class="px-6 py-3 text-right text-xs font-bold text-gray-500 dark:text-gray-300 uppercase tracking-wider">
                                <a href="{{ route('admin.faq.create') }}" class="inline-flex items-center px-3 py-1.5 bg-indigo-600 hover:bg-indigo-700 text-white font-semibold rounded-md shadow-sm transition duration-300 ease-in-out text-xs">
                                    Tambah
                                </a>
                            </th>
                        </tr>
                    ...
                    <tbody class="bg-white dark:bg-gray-800 divide-y divide-gray-200 dark:divide-gray-700">
                        @forelse ($faqs as $key => $faq)
                            <tr class="hover:bg-gray-50 dark:hover:bg-gray-700/50 transition duration-150">
                                    ...
                                    <div x-data="{ open: false }" @mouseenter="open = true" @mouseleave="open = false" class="relative inline-block text-left">
                                        ...
                                        <div x-show="open" 
                                            ...
                                            <div class="py-1" role="none">
                                                <a href="{{ route('admin.faq.edit', $faq) }}" class="text-gray-700 dark:text-gray-200 block px-4 py-2 text-sm hover:bg-gray-100 dark:hover:bg-gray-600" role="menuitem" tabindex="-1">
                                                    <i class="fas fa-edit w-5 mr-2"></i>Edit
                                                </a>
                                                <form action="{{ route('admin.faq.destroy', $faq) }}" method="POST" onsubmit="return confirm('Apakah Anda yakin ingin menghapus FAQ ini?');">
                                                    @csrf
                                                    @method('DELETE')
                                ...
                                {{-- Kolom kosong untuk menyejajarkan dengan tombol Tambah di header --}}
