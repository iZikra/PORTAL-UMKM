@props(['title' => 'Hapus Item', 'message' => 'Anda yakin ingin menghapus item ini? Tindakan ini tidak dapat diurungkan.'])

{{-- Komponen ini mengelola state-nya sendiri dan mendengarkan event global dari browser --}}
<div 
    x-data="{ showModal: false, deleteUrl: '' }"
    @open-delete-modal.window="showModal = true; deleteUrl = $event.detail.url"
...
>
    ...
    {{-- Konten Modal --}}
    <div x-show="showModal" x-transition:enter="ease-out duration-300" x-transition:enter-start="opacity-0 translate-y-4 sm:translate-y-0 sm:scale-95" x-transition:enter-end="opacity-100 translate-y-0 sm:scale-100" x-transition:leave="ease-in duration-200" x-transition:leave-start="opacity-100 translate-y-0 sm:scale-100" x-transition:leave-end="opacity-0 translate-y-4 sm:translate-y-0 sm:scale-95" class="fixed inset-0 z-10 overflow-y-auto">
