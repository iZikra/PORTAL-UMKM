    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div class="flex justify-between h-16">
                ...
                <div class="hidden space-x-8 sm:-my-px sm:ml-10 sm:flex">
                    @auth
                        @if (Auth::user()->role === 'admin')
                            ...
                            <x-nav-link :href="route('admin.faq.index')" :active="request()->routeIs('admin.faq.*')">{{ __('FAQ') }}</x-nav-link>
                        @else
                            <x-nav-link :href="route('dashboard')" :active="request()->routeIs('dashboard')">{{ __('Dashboard Saya') }}</x-nav-link>
                    ...
                    @else
                        ...
                        <x-nav-link :href="route('knowledge-base.public')" :active="request()->routeIs('knowledge-base.public')">Basis Pengetahuan</x-nav-link>
                        <x-nav-link :href="route('faq.public')" :active="request()->routeIs('faq.public')">FAQ</x-nav-link>
                    @endauth
                </div>
            </div>
            ...
            <div class="hidden sm:flex sm:items-center sm:ml-6">
                ...
                @else
                    <a href="{{ route('login') }}" class="text-sm font-medium text-gray-700 dark:text-gray-400 hover:text-gray-900 dark:hover:text-white">Log in</a>
                    <a href="{{ route('register') }}" class="ml-4 inline-flex items-center justify-center px-4 py-2 border border-transparent text-sm font-medium rounded-md shadow-sm text-white bg-indigo-600 hover:bg-indigo-700">Register</a>
                @endauth
            </div>
